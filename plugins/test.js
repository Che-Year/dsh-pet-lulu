 ...
 *
 * Run with:  node plugins/test.js
 * (requires plugins/node_modules/commander — see .gitignore)
 */
"use strict";
...
function main() {
  ...
  calls.length = 0;
  const sFile = path.join(tmpDir, "s.json");
  plugin.apply(makeCtx(["pet", "--pet-type", "capybara", "--fps", "20", "--gui",
                        "--status-source", "file", "--status-file", sFile]).ctx, {});
